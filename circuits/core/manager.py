...
class Manager(object):
    ...
    def getHandlers(self, event, channel):
        ...
        if channel_is_instance and channel != self:
            handlers = list(chain(self.getHandlers(event, None),
                         channel.getHandlers(event, channel)))
            return handlers
        ...
        for h in chain(*handlers_chain):
            if h.channel:
                handler_channel = h.channel
            elif hasattr(h, "__self__"):
                handler_channel = getattr(h.__self__, "channel", None)
            else:
