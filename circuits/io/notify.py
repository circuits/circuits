...
try:
    ...
    msg = 'No pyinotify support available. Is pyinotify installed?'
...
class Notify(BaseComponent):
    ...
    def _on_process_events(self, event) -> None:
        dir = event.dir
        mask = event.mask
        path = event.path
        name = event.name
    ...
    def add_path(self, path, mask=None, recursive=False, auto_add=True) -> None:
        mask = mask or MASK
        self._wm.add_watch(path, mask, rec=recursive, auto_add=auto_add)

    def remove_path(self, path, recursive=False) -> None:
    ...
    def _on_started(self, event, component) -> None:
        if self._poller is None:
            self._poller = Poller().register(self)
            self.fire(ready(self))
            event.stop()
