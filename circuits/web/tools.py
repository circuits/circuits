...
def validate_etags(request, response, autotags=False):
    ...
    if status.successful:
        if 'If-Match' in req.headers and not any(
            condition.matches(etag) for condition in req.headers.elements('If-Match')
        ):
            return httperror(
                request,
                response,
...
def digest_auth(request, response, realm, users):
    ...
    # inform the user-agent this path is protected
    response.headers["WWW-Authenticate"] = str(httoop.header.WWWAuthenticate('Digest', {
        'realm': realm.encode(),
        'algorithm': b'MD5',
        # 'domain': None,
        'nonce': httoop.authentication.digest.DigestAuthScheme.generate_nonce({'realm': realm.encode(), 'algorithm': b'MD5'}),
        # 'opaque': None,
        # 'stale': None,
        'qop': b'auth',
    }))
