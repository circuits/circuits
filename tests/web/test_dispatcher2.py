...
def test_simple(webapp):
    url = "%s/hello1" % webapp.server.base
    f = urlopen(url)
    s = f.read()
    assert s == b"hello1"
...
def test_expose(webapp):
    url = "%s/hello2" % webapp.server.base
    f = urlopen(url)
    s = f.read()
    assert s == b"hello2"
...
def test_controller_expose(webapp):
    url = "%s/hello/test" % webapp.server.base
    f = urlopen(url)
    s = f.read()
    assert s == b"hello test"

    url = "%s/world/test" % webapp.server.base
    f = urlopen(url)
    s = f.read()
...
def test_query(webapp):
    url = "%s/query?test=1" % webapp.server.base
    f = urlopen(url)
    s = f.read()
    assert s == b"query 1"

    url = "%s/hello/query?test=2" % webapp.server.base
    f = urlopen(url)
    s = f.read()
